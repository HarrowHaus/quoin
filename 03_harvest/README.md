...
**Status: shipped.** 40 packs landed in `packs/` — 30 token packs + 10
vocabulary packs — hitting the floor exactly. Per-pack mapping
decisions live in [`REPORT.md`](REPORT.md); a per-pack smoke gallery is
generated by [`smoke-gallery.js`](smoke-gallery.js).

## Token fidelity (Phase 3.5 + 3.5b)

Two follow-up phases upgraded token packs from "interpretation" to
"extraction". Phase 3.5 built the extraction framework and processed
6 packs; Phase 3.5b expanded the methodology to three methods and
brought the catalog to 27 of 30 packs extracted from canonical
upstream sources. See [`fidelity/README.md`](fidelity/README.md) for
the per-method guide.

For each token pack, a per-source extraction spec under
[`fidelity/specs/`](fidelity/) declares one or more methods:

- **Method A** — static fetch + format parser. New parsers added in
  3.5b: `primer-json5`, `value-wrapped-ts`.
- **Method B** — algorithm execution at extract time. Authorized to
  `npm install` source-system libraries as dev dependencies.
- **Method C** — per-file structured extraction. For sources whose
  values are static but split across files that defeat concatenated
  parsing.

Outcomes are stamped into each pack manifest as
`attribution.fidelityTier`:

- **Tier A** — byte-faithful extraction. No mapping interpretation.
- **Tier B** — extracted with mapping notes (granular palette
  flattened, etc.).
- **Tier C** — extraction not possible (no canonical source, license
  incompatibility, etc.).

**Final counts:** 1 Tier A, 26 Tier B, 3 Tier C. See
[REPORT.md](REPORT.md) for the per-pack table.

To run the extractor:

```bash
cd 03_harvest
npm install            # one-time, fetches culori + Method B libs
node fidelity/extract.js              # extract all specs
node fidelity/extract.js tailwind     # single pack
node build.js                         # regenerate packs from sources
node validate.js                      # confirm every pack still passes
```
