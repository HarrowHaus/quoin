...
## Token fidelity (Phase 3.5)

A follow-up phase (3.5) upgraded token packs from "interpretation" to
"extraction" where possible. For each token pack, a per-source
extraction spec under [`fidelity/specs/`](fidelity/) fetches the
canonical upstream source, parses values via format-specific parsers,
and converts to OKLCH at extraction time using
[culori](https://www.npmjs.com/package/culori). Outcomes are stamped
into each pack manifest as `attribution.fidelityTier`:

- **Tier A** — byte-faithful extraction. Values verbatim from source.
- **Tier B** — extracted with mapping notes (granular palette
  flattened onto Quoin's 4-surface model, missing tokens derived from
  base palette, etc.).
- **Tier C** — extraction deferred. Designed approximations stand,
  pending Phase 3.5b.

**Current counts:** 1 A, 5 B, 24 C. The Phase 3.5 stop condition
(>5 Tier C) is triggered — see [REPORT.md](REPORT.md) for the per-pack
table and the operator-ask block. The extraction framework is
reusable; Tier C packs can be promoted one at a time when their
upstream URLs and format-parser quirks are resolved.

To run the extractor:

```bash
cd 03_harvest
npm install            # one-time, fetches culori
