...
Ten universal layout + content primitives that the v1 vocabulary skipped.
Modern UI work — docs sites, marketing pages, app dashboards — needs all
of them; they belong in the reference set, not buried in a per-domain
harvested pack.
...
| Primitive | Role |
|-----------|------|
| `<tab-panels>` | Full tab system: tab row + active panel content. |
| `<pin-entry>` | Multi-cell numeric / OTP input (4–6 connected cells). |
| `<stat-display>` | Big number + label, optional delta indicator. |
| `<split-shell>` | Fixed sidebar + flexible main content area. |
| `<columns>` | Explicit responsive columns (1:1, 1:2, 2:1) that collapse on small screens. |
...
This pack is in the v1 reference floor alongside `@quoin/vocab-editorial`
and `@quoin/vocab-dashboard`. Load all three together for the full
